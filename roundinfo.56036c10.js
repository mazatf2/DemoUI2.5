...
const TableRow = round => {
  ...
  const cells = [_utils.strings.get(type), start, end, round?.firstDeath?.tick || '', round?.midCapture?.tick || ''];
  const pauses = round.pauseList.map(i => html`
		<tr>
			<td>${_utils.strings.get(i.type)}</td>
			<td>${i.start}</td>
			<td>${i.end}</td>
			<td></td>
			<td></td>
...
define('page-roundinfo', {
  ...
  async update() {
    const {
      rounds,
      users
    } = await getRounds(this.arrayBuffer);
    console.log(rounds);
    const options = {
      showSpoilers: true,
      scalePx: 16
